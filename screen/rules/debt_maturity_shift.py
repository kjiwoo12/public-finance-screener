...
# 위 한 줄을 회계를 모르는 사람에게 다시 설명한 것. 조서에 둘 다 낸다.
# 서식을 풀어 버리면 조서가 아니게 되고, 용어만 두면 읽을 사람이 감사인으로
# 좁혀진다. 둘 다 필요하다.
# *별표* 로 감싼 곳을 화면에서 강조한다. 규칙이 HTML 을 들고 있으면 출력
# 형식이 바뀔 때마다 규칙을 고쳐야 한다.
PLAIN = ("빌린 돈의 *총액은 비슷한데 갚아야 할 시점이 1년 안쪽으로 몰린* "
         "해를 찾는다. 빚이 늘어난 것이 아니라 *기한이 당겨진* 것이다. "
         "그래서 빚이 얼마인지만 보면 아무 일도 없어 보인다.")

# 이 절차가 쓰는 말. 조서를 처음 읽는 사람이 여기서 막힌다.
TERMS = [
    ("유동부채", "1년 안에 갚아야 하는 빚"),
    ("고정부채", "갚을 때까지 1년 넘게 남은 빚"),
    ("만기 도래", "갚을 날이 다가와 자연히 유동부채로 옮겨지는 것. 정상이다"),
    ("차환", "만기가 된 빚을 새로 빌려 갚는 것. 실패하면 급하게 돈을 구해야 한다"),
]
THRESHOLDS = [
